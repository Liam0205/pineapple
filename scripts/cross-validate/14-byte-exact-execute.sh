#!/usr/bin/env bash
# Section 14: Byte-exact server /execute JSON parity (Go vs Java vs C++)
#
...
echo
echo "==> [14/$TOTAL_SECTIONS] Byte-exact /execute JSON parity (Go vs Java vs C++)"
...
" 2>/dev/null || { fail "byte-exact: parse $fname"; continue; }
  ...
  bx_cleanup() {
    ...
    for port in $GO_PORT $JAVA_PORT $CPP_PORT; do
