...
# run_engines_parallel config request [res_args...]
# Runs Go, Java, Python on the same config+request in parallel.
# Results are written to $1.go.out, $1.java.out, $1.py.out
# Exit codes written to $1.go.rc, $1.java.rc, $1.py.rc
run_engines_parallel() {
  ...
  (cd "$REPO_ROOT/pine-python" && python3 -m pine.cli.run -config "$config" -request "$request" "${py_res_args[@]}") \
