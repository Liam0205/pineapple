...
class Closer:
    ...
    teardown (e.g. a pool of Lua interpreter states). The engine calls close()
    on every operator that implements it when the engine is retired — during a
    config hot-reload, or on shutdown — so a swapped-out engine does not leak
    its operators' resources. close() must be safe to call once; the engine does
    not call it twice on the same instance.
    """
