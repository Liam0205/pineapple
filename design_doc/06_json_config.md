...
| `recall` | 否 | 标记为召回算子，结果暂存而非写入主 DataFrame |
...
1. 识别 `"recall": true` 的算子，执行后将结果存入引擎内部暂存区（不写主 DataFrame）。
2. 识别 merge 算子的 `"sources"` 字段，建立 source → merge 的显式 DAG 边（不走字段名推导）。
3. merge 执行时从暂存区取出各路结果，按策略去重后填充主 DataFrame 的 item 表。
4. 暂存区随后由引擎回收。
...
每个条件分支被编译为一个 Lua 控制算子（`for_branch_control: true`），输出一个隐藏的控制属性。分支内的算子通过 `skip` 引用该控制属性，控制属性为 true 时跳过。

**skip 语义约定：控制属性值为 true 表示"跳过"，false 表示"执行"。**
...
嵌套 if 天然支持——每层 if 生成独立的控制属性命名空间（由 Apple 自动分配不同编号），内层分支的 `skip` 同时引用外层和内层的控制属性。
...
`inner_op` 的 skip 需要外层和内层都"不跳过"才执行。Apple 在编译时为内层算子注入外层控制属性的依赖。具体实现方式：内层算子同时引用外层和内层控制属性，引擎无需特殊处理——多个 skip 条件中任一为 true 即跳过。
...
5. 对 merge 算子的 `sources` 字段，建立 source → merge 的显式 DAG 边。
6. 基于 DAG 拓扑排序并行调度。
