...
条件表达式中的字段引用使用 `{{field_name}}` 模板语法标记，编译器据此提取字段依赖并生成 `common_input`；双花括号之外的部分作为原生 Lua 表达式保留。

### 基本原理

每个条件分支被编译为一个 Lua 控制算子（`for_branch_control: true`），输出一个隐藏的控制属性（common 字段，`_` 前缀）。分支内的算子通过 `skip` 字段引用该控制属性的字段名列表，Pine 在执行前依次读取列表中每个字段值，任一为 true 时跳过算子。

**skip 语义约定：`skip` 是一个 common 字段名列表。运行时任一字段值为 true 表示"跳过"，全部为 false 表示"执行"。** 单层分支中列表通常只有一个元素；当 SubFlow 嵌套在外层分支内时，列表包含多个字段名，实现多层保护。
...
分支内可以包含 SubFlow。编译器在遍历 SubFlow 子树时，将外层分支的控制字段追加到 SubFlow 内每个算子的 `skip` 列表和 `common_input` 中。SubFlow 内部如果有自己的控制流，其控制字段名会被加上 SubFlow 路径前缀以避免与外层同名冲突。

```python
sf = SubFlow(name="ranking")
sf.reorder_sort(item_input=["item_score"], field="item_score", order="desc")
...
编译结果中，`reorder_sort` 的 `skip` 为 `["_if_1"]`，与直接写在分支内的算子行为一致。若 SubFlow 内部还有 `if_`，其内层控制字段会被命名为 `_ranking_if_1`（SubFlow 路径作为前缀），内层算子的 `skip` 包含 `["_ranking_if_1", "_if_1"]`，同时受内外两层保护。
