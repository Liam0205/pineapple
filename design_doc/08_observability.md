...
三运行时（Go `pkg/metrics` Collector+Tee、Java `MetricsCollector`+`TeeProvider`、C++ `metrics::Collector`+`metrics::TeeProvider`）行为字节级对齐，由 cross-validate section 16 锁定（正例 4 指标 + `up=1` + ping count≥1，负例空 `metrics_name` → `resources == {}`）。
