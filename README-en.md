...
21. **storage_mode validation parity** — a valid `storage_mode` is accepted by all three with byte-identical output, and an invalid one is rejected by all three (issue #187; the dispatch direction itself is pinned by each runtime's factory unit test, see `llmdoc/architecture/dag-engine.md`)
