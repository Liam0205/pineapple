...
issue #179 的 cross-validate section 21 是正面样本：它只能钉「非法值被静默接受 + 三方输出字节相同」，抓不到 dispatch 分歧本身——实测把 pine-cpp 的 fallback 改回去，section 21 仍然全绿（mutation 验证过）。脚本注释显式写了这三件事，以及真正的门在哪三个单测文件里。section 21 仍然有价值，但价值不是「抓 #179」：它守的是「未来某一侧单独加上拒绝非法值时会立刻红」这条负空间。
