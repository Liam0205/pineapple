...
2. **`[[ $x == +([0-9]) ]]` 通过 ≠ `$(( x ))` 给你期望的值**：该模式接受前导零，而 bash 算术把前导零读作八进制。两种失败形式不同，值得分清：`010m` 在函数内部算错、静默变成 480s 而非 600s；`0900s` 走的分支不做算术，原始字符串逃到调用方才炸，`DEADLINE` 因此未赋值、`set -u` 直接中止脚本——**连"预算不足"那条守卫都来不及报**（而 `08m` 是在函数内部炸的，守卫报了，只是理由写错）。所有进入算术的分支都要 `10#`，且返回的必须是算术结果而非原始字符串。
