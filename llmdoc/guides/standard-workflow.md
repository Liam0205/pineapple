...
- **新公开入口上线后，审计既有全局状态的归属**：状态机重扫之外，还要扫进程级状态——`sync.Once`、静态 CAS、System property、全局 setter 这类形式都是为单实例进程设计的；嵌入/多实例 API 让多引擎/多服务器成为一等公民后，逐个回答"这个状态该属于谁（进程还是实例）"。已知三例均由嵌入 API 暴露（issue #169 / #172）：Java `PineServer` stop 后状态可达（见上条）、按值 `Config` 的 Addr 默认值副本回归（见 `memory/reflections/upstream-serverplus-custom-routes.md`）、Go `log_prefix` 的包级 `sync.Once` first-engine-wins——后到引擎的前缀被静默忽略、日志带别人的前缀（修复为引擎实例级 logger，见 `memory/reflections/per-engine-log-prefix.md`）。
...
- 示例二进制纳入默认构建（如 CMake 默认 target）防 rot
