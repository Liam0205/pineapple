...
改对实现之后既存用例变红，**第一步不是把实现改回去，而是确定基准是哪一方**。判据是「这条断言引用的那个运行时，实测行为是什么」，不是「注释怎么说」。

issue #179 的实例：`pine-cpp/tests/test_row_frame.cpp` 原有一行断言非法 `storage_mode` 不会得到 `RowFrame`，还附注释 `// defaults to column` 解释为什么应该这样——而 pine-go 的兜底是 row。**这比没有测试更糟：它让改对的人以为自己改坏了**，第一反应就是回退实现。

同一 issue 的另一半：契约定义方 pine-go 的 `default` 分支**原本连测试都没有**，规则只隐含在 `switch` 的结构里。参考行为没有可引用的断言点，是复刻方各自解读出不同兜底方向的条件之一。因此**给参考实现补上覆盖该分支的用例，与修复复刻方同等重要**。

### 一条描述连续两轮被改错，就把它写成用例

文字描述跨运行时行为差异时，容易把**机制**写成**类别**，而类别几乎总是错的。issue #187 的审计里
「pine-java 什么时候跟 pine-go 一致」这一条连续四轮没写对：
...
### 修错误注释要按「这条表述出现在哪里」清理

修一条错误注释或错误文档表述时，**按「这个错误声明可能出现在哪里」搜全仓，而不是按「issue 或审查指了哪几个文件」搜**。

issue #179 把这条纪律逐级放宽了两次，每次都是被下一轮审计打回来的：

| 轮次 | 当时的搜索范围 | 漏掉的那份 |
|---|---|---|
| 初版 | issue 点名的两处（`row_frame.cpp`、`frame.hpp:114`） | `frame.hpp:24` 的文件头概览（同文件，差 90 行） |
| 审计第一轮 | 「同文件内搜完」 | `engine.cpp:1236`，`make_frame` 唯一生产调用点的上一行 |
| 审计第二轮 | 「搜全仓的代码」 | `architecture/pine-cpp-runtime.md:104`，llmdoc 里的第五份 |

教训不是「搜得再宽一点」，而是：**这类声明会同时存在于代码注释、脚本注释、用户文档、架构文档四层**，
...
两次都是审计下一轮抓出来的，第二次的 commit message 还刚把第一次列为「本轮已修的 important」。
判据：**编辑跨行的引文或表格后，读一遍渲染后的整段，并检查引号成对。** 正则与按行替换都看不见
「这一句从哪里开始、到哪里结束」，而引文恰恰是最需要这个边界的东西。

另有一类相关失效：**本次代码改动会让别处原本正确的注释变成假声明。**
`scripts/bench-cross-runtime.sh` 里「三方连兜底方向都不一致」这句在 base 上准确，是 #179 的代码改动
...
- issue #183：pine-cpp 的 JSON key 顺序「天然满足」只对走 `Variant` writer 的路径成立（`std::sort` + `std::string` 的 `<` 即字节序），`/stats` 是手写拼接 JSON、三处都错。「某个运行时天然满足」只对具体代码路径成立，不对整个运行时成立。
- issue #187（同型第二例，字段粒度）：pine-cpp 对四个同类根级字符串字段有**三种**处理——`storage_mode` 调 throwing 的 `as_string()` 抛错，`log_prefix` 与两个 `_PINEAPPLE_*` 有 `is_string()` 守卫、静默忽略类型错误。所以「pine-cpp 拒绝非法类型」这句话在字段粒度上是错的。
