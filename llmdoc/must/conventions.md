...
算子契约源自 `operators/` 下的 Go 注册 + `internal/types/operator.go` 和 `internal/registry/registry.go`。Python DSL 和生成的 helper 消费这些契约但不覆盖它们。Pine-Java 实现等效语义但不引入新的 Schema 定义来源。
...
- 生成的 Python 类应视为派生输出
- Markdown 算子文档也是派生输出
- Java 侧新增算子应确保与 Go Schema 对齐后再注册
