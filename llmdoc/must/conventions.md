...
跨 Python/Go/Java/Python-runtime/C++ 边界的变更应优先保持或有意演进该 JSON 契约，而非引入运行时桥接。规划中的 pine-cpp 也必须服从这条原则。
...
## 现有运行时的独立 Schema 源 + CI 十三层交叉验证

Go、Java 和 Python 各自维护独立的算子 Schema 注册表，互不依赖。三者在 CI 中通过 `scripts/cross-validate.sh` 的十三层交叉验证保持对齐：
...
12. **Extensibility-parity 层**：未注册路径 404 JSON parity、前缀匹配泄漏检查、Content-Type 一致性
13. **Metrics-parity 层**：zero-traffic pre-init、operator names match、exec/skip/error_count match、scheduler.run_count match
...
5. Go/Java/Python 十三层跨验证测试（`scripts/cross-validate.sh`），覆盖 codegen-schema、render、execution、column-store、error、server HTTP、cancellation、concurrent、raw-byte、hot-reload、redis-integration、extensibility-parity、metrics-parity
