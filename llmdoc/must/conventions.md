...
`apple/` 中的 Apple DSL 声明流水线，但 Go 引擎和 Java 引擎仅消费符合 `pine-go/internal/config/types.go` 结构的 JSON。该 JSON 是以下场景的解耦边界：
...
## Go/Java 双独立 Schema 源 + CI 多层交叉验证

Go 和 Java 各自维护独立的算子 Schema 注册表，互不依赖。两者在 CI 中通过 `scripts/cross-validate.sh` 的 11 层交叉验证保持对齐。脚本已模块化为 `scripts/cross-validate/` 目录下的独立模块：

1. **Schema + Codegen 层** (`01-codegen-schema.sh`)：双侧各自导出 JSON（Go: `pineapple-codegen -schema-json`; Java: `Codegen --export-schema`），CI 执行 normalized diff；同时比对生成的 Python DSL 产物字节级一致性
2. **Render-DAG 层** (`02-render-dag.sh`)：共享 `fixtures/pipelines/`，双侧渲染 DOT/Mermaid/Collapsed 输出并做字节级 diff
3. **Execution 层** (`03-execution-parity.sh`)：共享 `fixtures/pipelines/`，双侧执行同一 fixture 并比对 JSON 输出
4. **Column-store 层** (`04-column-store.sh`)：相同 fixture 在 `storage_mode=column` 下双侧执行并比对
5. **Error 层** (`05-error-parity.sh`)：共享 `fixtures/errors/`，双侧执行错误路径 fixture 并比对错误输出格式
6. **Server HTTP 层** (`06-server-http.sh`)：17 项端点行为 parity（health、execute、stats、dag、413 body limit、cancellation 等）
7. **Cancellation 层** (`07-cancellation.sh`)：超时和取消信号传播行为双侧对比
8. **Concurrent 层** (`08-concurrent.sh`)：并发执行行为一致性验证
9. **Raw-byte 层** (`09-raw-byte.sh`)：原始字节输出格式对比
10. **Hot-reload 层** (`10-hot-reload.sh`)：配置热加载行为双侧对比
11. **Redis-integration 层** (`11-redis-integration.sh`)：Redis 算子集成行为双侧对比

入口脚本 `scripts/cross-validate.sh` 支持选择性执行：`bash scripts/cross-validate.sh 1-5,8,10-11`。共享环境变量和预构建逻辑分别位于 `_env.sh` 和 `_prebuild.sh`。
...
- 共享 fixture 位于仓库根 `fixtures/` 目录（三子目录：`operators/`、`pipelines/`、`errors/`），双侧通过相对路径访问
...
5. Go/Java 11 层跨验证测试（`scripts/cross-validate.sh` + `scripts/cross-validate/`），覆盖 schema、render、execution、column-store、error、server HTTP、cancellation、concurrent、raw-byte、hot-reload、redis-integration
...
Go 侧 `pine-go/cmd/pineapple-codegen/main.go` 从自身注册表生成；Java 侧 `Codegen.java` 支持双模式（`--schema-from-registry` 从内部 Registry 生成，`-schema <path>` 从外部 JSON 生成）。
