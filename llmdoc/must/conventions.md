...
## Go/Java/Python 三独立 Schema 源 + CI 十二层交叉验证

Go、Java 和 Python 各自维护独立的算子 Schema 注册表，互不依赖。三者在 CI 中通过 `scripts/cross-validate.sh` 的十二层交叉验证保持对齐：
...
5. Go/Java/Python 十二层跨验证测试（`scripts/cross-validate.sh`），覆盖 codegen-schema、render、execution、column-store、error、server HTTP、cancellation、concurrent、raw-byte、hot-reload、redis-integration、extensibility-parity
