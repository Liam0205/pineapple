...
## Go/Java 双独立 Schema 源 + CI 七层交叉验证

Go 和 Java 各自维护独立的算子 Schema 注册表，互不依赖。两者在 CI 中通过 `scripts/cross-validate.sh` 的七层交叉验证保持对齐：

1. **Schema + Codegen 层**：双侧各自导出 JSON（Go: `pineapple-codegen -schema-json`; Java: `Codegen --export-schema`），CI 执行 normalized diff；同时比对生成的 Python DSL 产物字节级一致性
2. **Render-DAG 层**：共享 `fixtures/pipelines/`，双侧渲染 DOT/Mermaid/Collapsed 输出并做字节级 diff
3. **Execution 层**：共享 `fixtures/pipelines/`，双侧执行同一 fixture 并比对 JSON 输出
4. **Column-store 层**：相同 fixture 在 `storage_mode=column` 下双侧执行并比对
5. **Error 层**：共享 `fixtures/errors/`，双侧执行错误路径 fixture 并比对错误输出格式
6. **Server HTTP 层**：17 项端点行为 parity（health、execute、stats、dag、413 body limit、cancellation 等）
7. **Cancellation 层**：超时和取消信号传播行为双侧对比
...
5. Go/Java 七层跨验证测试（`scripts/cross-validate.sh`），覆盖 schema、render、execution、column-store、error、server HTTP、cancellation
