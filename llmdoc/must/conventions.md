...
四次同型教训，只是投影方向不同：#180 把范围说小一个量级（实测 15/20 分歧、缺陷分布在两个运行时）、#183 同时说大又说小（`/execute` 上只有一个运行时错、但 `/stats` 上另一个也错）、#179 issue 写得细、实测确认后按其范围推进（做对了的一次）、#187 issue 是自己写的、仍然把范围写窄（说的是一个字段的值校验，实测是四个字段的类型规则加一个字段的值白名单）。
