...
pine-cpp 提供 `pine::server::http_metrics_middleware(provider)` 工厂返回同名指标的 `Middleware`：

- 指标名、标签集合、histogram bucket 与 Go 端字节级一致
- 与 Go 不同，C++ 端**不会自动包装**：用户必须显式 `cfg.middlewares.push_back(pine::server::http_metrics_middleware(provider))`，未显式注入则不产生 HTTP 层指标
- `MiddlewareContext::status` 通过 `send_response` 内的 `thread_local` 指针写回，使外层 middleware 能在 `next()` 返回后读取实际响应状态码
- `Provider*` 由调用方持有，必须保证生命周期覆盖 server 运行期
