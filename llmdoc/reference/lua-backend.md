...
### SetGlobal 键解析快路径（GlobalsSlot 缓存，v0.2.0-rc4 起，#115 finding B）

与上述 pin 所有权**正交**的一条 perf 路径，针对 `executeForItem` 热路径——N items × M ItemInput 字段即每请求 N×M 次 `Engine.SetGlobal`，name 取自固定的 M 个字段名。旧路径每次都走 `st.SetGlobal(name, value)`，内部对同一 name 反复 `[]byte(name)` + intern 哈希查找 N 次。

下游在 `wangshuEngine` 上加 borrow 作用域的 `map[string]wangshu.GlobalsSlot`，把 `SetGlobal` 改路由 `SetBySlot`：

- 首次见到某 name 调 `st.GlobalsSlot(name)` 预解析并 pin key、缓存 slot
- 后续同名直接 `st.SetBySlot(slot, value)`，省去 per-call intern
- `releaseSlots()` 在 `wangshuPool.Return` 中紧挨 `releaseFnCache` 执行，`Release` 每个 slot 后 `e.slots = nil`（#116 finding 2 把旧 `(&s).Release()` + delete 改为显式清空，对齐 `SetGlobal` 的 `if e.slots == nil` 惰性初始化契约），避免 pin 槽跨借用累积——符合 wangshu GlobalsSlot 文档「反复创建大量不同 name 的 slot 时应配套 Release」的约束

此改动 wangshu-internal：`Engine` 接口不变（LuaOp 不动）、gopher-lua 侧无预解析键快路径仍走原 `SetGlobal`、跨引擎 `lua_script` 字节对等契约不受影响。
...
- **typed-array 快路径**（v0.2.0-rc4 起，#115 finding A + #116 finding 1）：当 `[]any` 全部元素同属 wangshu 四种原生标量类型（`float64` / `int64` / `bool` / `string`，外加 Go-native `int`——#116 补齐，按 int64 处理）时，`tryTypedArray` 经一次线性分类走对应 `NewFloatArrayTable` / `NewInt64ArrayTable` / `NewBoolArrayTable` / `NewStringArrayTable` 批量建表，省去逐元素 `toValue` type-switch 与 `[]wangshu.Value` 中间物化。这正是 `executeForCommon` 喂入的热形状——DataFrame 列 lift 成底层类型一致的 `[]any`
- **回退路径**（v0.2.0-rc3 起）：异质 / 含 nil / 复合元素仍走 `State.NewArrayTable(vals)` 一次性建表。`NewInt64ArrayTable` 在 `|v| > 2^53` 时静默回退，保留异质路径 int64→float64 有损转换的历史行为
- 两条路径都只让脚本看到普通 Lua 数组表（`xs[i]` 下标），**不是** arena 列轨 proxy，跨引擎 `lua_script` 契约不变
