...
- `bench-cross-runtime.sh` — 跨运行时 benchmark（四引擎 × 6 档 DAG 规模，三阶段：顺序延迟 / 固定 QPS / 最大吞吐）
...
CI 在 `.github/workflows/ci.yml` 中运行各运行时测试、codegen 新鲜度检查、cross-validate 与差异模糊测试。Nightly benchmark（`.github/workflows/nightly-benchmark.yml`）每日对比四运行时性能并生成 delta 报告。发布在 `.github/workflows/release.yml` 中在 `v*` 标签上增加 wheel 构建和 PyPI 发布。
