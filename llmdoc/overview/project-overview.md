...
三层通过 JSON 解耦。Python 侧不在运行时调用 Go/Java，Go/Java 侧不知道 Python 对象。契约是各引擎 `Engine.create()`/`pine.NewEngine()` 消费的配置格式。
...
这也解释了 `cmd/pineapple-codegen/main.go` 和 `pine-java/Codegen.java` 的存在：Go 算子 Schema 是类型化 helper 和生成算子文档的唯一事实源，Java 侧的 Codegen 可从同一 Schema JSON 生成等效的 Python helper 和文档。
...
这就是跨语言测试使用 `testdata/e2e_apple_dsl.json` 等文件而非直接桥接的原因。Pine-Java 的 `FixtureTest` / `PipelineFixtureTest` 同样消费 Go 侧的 `testdata/` fixture 做对等验证。
...
- `pine-java/src/test/` 中的 Java 引擎测试，包括 fixture 对等验证和 benchmark
