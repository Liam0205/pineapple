...
Pineapple 是一个面向请求时数据处理的高性能流水线引擎。流水线通过 Python Apple DSL 声明，编译为 JSON 配置，由 Go / Java / Python / C++ 各运行时构建依赖感知的 DAG 并执行。当前仓库采用一个声明层 + 四个运行时目录的布局：`apple/`（Python DSL）、`pine-go/`（Go 引擎）、`pine-java/`（Java 引擎）、`pine-python/`（Python 引擎）、`pine-cpp/`（C++ 引擎，定位为完全 parity 前提下的标杆运行时）。
...
- `pine-python/` 中的 Python 执行层（包名 `pineapple-pine`），功能对等的运行时，使用 lupa/LuaJIT 执行 Lua 脚本、ThreadPoolExecutor 调度 DAG、mtime-polling 实现热加载。
- `pine-cpp/` 中的 C++ 执行层，CMake + C++23，包含 `ColumnFrame` 列存、per-node thread DAG 调度、LuaJIT 集成、`metrics::Provider` 抽象、`resource::Manager` 后台刷新与 HTTP middleware 注入。详见 `architecture/pine-cpp-runtime.md`。

各运行时之间通过 JSON 解耦。Python DSL 不在运行时调用任一引擎，各引擎侧不知道 Python DSL 对象。契约是各引擎 `Engine.create()` / `pine.NewEngine()` / `Engine(json_config)` / `pine::Engine::from_file(...)` 共同消费的配置格式。Go 模块路径为 `github.com/Liam0205/pineapple/pine-go`。
...
这也解释了各运行时各自维护 codegen 入口（`pine-go/cmd/pineapple-codegen/main.go` / `pine-java/Codegen.java` / `pine.cli.codegen` / `pineapple-cpp-codegen`）的存在：算子 Schema 在每个运行时独立注册，CI cross-validate 做最终对齐。
...
- `pineapple-cpp-server -config <pipeline.json> [-addr :8080] [-read-header-timeout 10s] [-read-timeout 30s] [-write-timeout 60s] [-idle-timeout 120s] [-max-body-size 10485760]` — HTTP 服务，支持 graceful shutdown、配置 mtime 热加载、`ServerConfig::middlewares` 注入与 `pine::server::http_metrics_middleware(provider)` 内置指标 middleware
...
这就是跨语言测试使用 `pine-go/testdata/e2e_apple_dsl.json` 等文件而非直接桥接的原因。共享 fixture 位于仓库根 `fixtures/` 目录（子目录：`operators/` 单算子、`pipelines/` 端到端管道、`errors/` 错误路径）。Go、Java、Python、C++ 引擎测试均从同一路径读取。
...
Java / Python / C++ 各自维护独立的 Schema 注册表（`Registry.register` / `pine.registry` / `pine::register_operator`），均可独立产出 codegen。CI cross-validate 的 codegen-schema section 通过 normalized diff 验证四套 Schema 一致。

Python DSL 消费这些契约但不重新定义它们。各运行时实现等效语义但不引入新的对外 Schema 事实源。
...
CI 在 `.github/workflows/ci.yml` 中运行各运行时测试、codegen 新鲜度检查、cross-validate 与差异模糊测试。发布在 `.github/workflows/release.yml` 中在 `v*` 标签上增加 wheel 构建和 PyPI 发布。

## 项目边界

Pineapple 当前不包括：

- 直接的 Python→任一引擎运行时桥接（始终通过 JSON）
