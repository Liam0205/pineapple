...
- Pine-Python 侧维护独立注册表，通过 CI 十一层交叉验证保持对齐
...
- `cross-validate.sh` — Go vs Java vs Python 十一层跨验证
...
CI 在 `.github/workflows/ci.yml` 中运行 Go 测试、Python 测试、Java 测试、Python 引擎测试、codegen 新鲜度检查、十一层三引擎跨验证和差异模糊测试。发布在 `.github/workflows/release.yml` 中在 `v*` 标签上增加 wheel 构建和 PyPI 发布。
