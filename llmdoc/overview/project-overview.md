...
Pineapple 是一个面向请求时数据处理的高性能流水线引擎。流水线通过 Python Apple DSL 声明，编译为 JSON 配置，由 Go/Java 运行时构建依赖感知的 DAG 并执行。仓库采用三对等目录布局：`apple/`（Python DSL）、`pine-go/`（Go 引擎）、`pine-java/`（Java 引擎）。
...
三层通过 JSON 解耦。Python 侧不在运行时调用 Go/Java，Go/Java 侧不知道 Python 对象。契约是各引擎 `Engine.create()`/`pine.NewEngine()` 消费的配置格式。Go 模块路径为 `github.com/Liam0205/pineapple/pine-go`。
...
4. 在 `pine-go/internal/dag/` 中根据屏障、数据冒险和显式 merge source 构建 DAG。
...
`scripts/` 提供标准化脚本覆盖完整开发流程：

- `apple-compile.sh` — 编译 Apple DSL 为 JSON
- `codegen.sh` — 从 Registry 生成 Python DSL 代码
- `cross-validate.sh` — Go vs Java 11 层跨验证（模块化为 `scripts/cross-validate/` 子目录）
