...
- `pine-python/pine/dag.py`
- `pine-python/pine/engine.py`
...
逐算子 debug 同样采用 nullable 三态（Go `*bool`、Java `Boolean`、Python `Optional[bool]`、C++ `std::optional<bool>`）：未设置时继承 flow-level 全局 debug；显式设置时覆盖全局值。因此全局 debug 不再单向覆写所有算子的 `opCfg.Debug`，而是仅作为未显式设置 debug 的算子的默认值。
...
Go 侧在 `pine.NewEngine()` 编译算子时，通过 interface 类型断言（`types.ConsumesRowSet`、`types.MutatesRowSet`、`types.AdditiveWritesRowSet`）检测标记，并写入 `OperatorConfig` 的对应布尔字段。Python 引擎侧使用 mixin class 继承检测。Java 侧使用同名 interface。
...
- 四运行时的 ExecutionError/PanicError 均**保留原始异常因果链**：Go 通过 `Unwrap()` 接口，Java 通过 `Throwable.getCause()`，Python 通过 `__cause__` 属性，C++ 通过 `std::nested_exception`（多继承）+ `pine::error_as<T>()` helper。Section 15（`scripts/cross-validate/15-error-cause-chain.sh`）通过四方 probe binary 字节级验证四方 cause chain 输出 `"PASS:key=user:42 not found"` 一致
...
经 `ExecutionError` 包装后最终输出形如 `pine: execution error in operator "op": required field "X" is missing in common`，由 `fixtures/errors/runtime_build_input_missing_field.json` 通过 `wrapping_exact` 锁定四运行时（go/java/cpp）的字节级一致（python 走相同语义但 wrapping prefix 列表以 fixture 为准）。
...
- **Engine 退役统一走 `Engine.Close()`**：四运行时（pine-go `Engine.Close()`、pine-java `Engine.close()`、pine-python `Engine.close()`、pine-cpp `Engine::close()`）在 hot-reload swap 与 graceful shutdown 时遍历 `CompiledOperator.Instance`，对实现 `Closer`（详见 `llmdoc/reference/operator-contract.md`）的算子触发 `Close()`，单算子失败聚合上报但不阻断后续算子。各 server 实现在锁外触发旧引擎的 `Close()`，避免阻塞 reload 路径。
...
12. **全局 debug 通过 nullable 三态继承生效。** 根级 `debug` 与 `pine.WithDebug(...)` 确定 flow-level 全局 debug 默认值；逐算子 `debug` 为 nullable 三态（Go `*bool`、Java `Boolean`、Python `Optional[bool]`、C++ `std::optional<bool>`），未设置时继承全局值，显式设置时覆盖全局值。Go option 还必须保留”未设置”与”显式 false”两种状态区分。
...
- Transform: `transform_copy`、`transform_dispatch`、`transform_normalize`、`transform_size`、`transform_by_lua`、`transform_resource_lookup`、`transform_redis_get`、`transform_redis_set`、`transform_by_remote_pineapple`、`transform_bench_cpu`、`transform_bench_sleep`（后两者仅供 benchmark 使用，pine-python 不实现）
...
`pine-python/`（包名 `pineapple-pine`）提供完全对等的 Python 运行时，共享同一 JSON 配置格式。

核心实现：

- **引擎编译流水线**：JSON 解析 → 算子序列展开（含 SubFlow）→ 标记接口检测（`ConsumesRowSet`/`MutatesRowSet`/`AdditiveWritesRowSet`）+ item 字段 auto-inject → DAG 推导
- **DAG 推导**：与 Go/Java 相同的三阶段算法（数据冒险 + 显式 sources + 传递性归约 + 拓扑排序），通过 `_row_set_` 哨兵、三标记和 auto-inject 实现行集依赖
- **并发调度**：`ThreadPoolExecutor` + `concurrent.futures` 事件驱动，语义等同 Go 的 per-operator goroutine
- **DataFrame**：`RowFrame` 和 `ColumnFrame`，通过 `storage_mode` 配置选择
- **Lua 执行**：lupa/LuaJIT 沙箱，池化复用，baseline snapshot/restore
- **热加载**：mtime-polling 检测文件变更，原子快照替换
- **全部内置算子**：与 Go/Java 完全对齐（pine-python 不含 `transform_bench_cpu` / `transform_bench_sleep` 两个 benchmark-only 算子）
- **Strict/Default field accessor**：与 Go/Java 对等的字段访问策略
- **CLI**：`run`、`server`、`dag`、`codegen` 四个子命令

### Pine-Python 检索指针

- 引擎编译和请求生命周期：`pine-python/pine/engine.py`
- 配置解析：`pine-python/pine/config.py`
- DAG 推导：`pine-python/pine/dag.py`
- DataFrame：`pine-python/pine/dataframe.py`
- 算子基类和标记接口：`pine-python/pine/operator.py`
