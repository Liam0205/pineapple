...
- 双通道观测：同一执行路径同时更新原子统计（`internal/runtime/stats.go`）与外部 Provider metrics（`internal/runtime/engine_metrics.go`）
...
`internal/runtime/engine_metrics.go` 在 `NewEngine` 时预创建 scheduler 级 metric handle，避免请求热路径反复声明 metric。
...
`internal/types/errors.go` (`PanicError`) 现在区分面向外部的错误消息与面向内部的诊断信息：
...
`internal/runtime/stats.go` 维护进程内累计统计：
...
`pkg/server/server.go` 还维护服务端级 reload 观测：
...
- `pkg/server.Config.Metrics` 把同一个 `pkg/metrics.Provider` 同时传给 server reload 观测、内置 HTTP 请求指标中间件和 `pine.NewEngine(..., pine.WithMetrics(provider))`，形成统一的外部指标出口。
- `pkg/server/server.go` 内置安全加固：
...
- 引擎级指标句柄：`internal/runtime/engine_metrics.go`
- 可插拔指标接口：`pkg/metrics/metrics.go`、`pkg/metrics/nop.go`
- HTTP 服务与 `/stats`：`pkg/server/server.go`
