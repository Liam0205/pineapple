...
优先级同样固定为 Go option 高于 JSON 配置，但实现上需要额外区分”未设置 option”和”显式传入 false”。因此 `WithDebug` 在 `NewEngine` 内部通过 `*bool` 风格状态表达三态：未设置时沿用 JSON，显式 `true/false` 时覆盖 JSON。最终值一旦确定，`NewEngine` 会把它作为 flow-level 全局 debug 默认值。

逐算子 debug 同样采用 nullable 三态（Go `*bool`、Java `Boolean`、C++ `std::optional<bool>`）：未设置时继承 flow-level 全局 debug；显式设置时覆盖全局值。因此全局 debug 不再单向覆写所有算子的 `opCfg.Debug`，而是仅作为未显式设置 debug 的算子的默认值。
...
4. **构建算子实例**（`pine-go/internal/registry.BuildOperator`）
   ...
   - 对 `MetadataAware`、`DebugAware`、`LoggerAware`、`MetricsAware` 算子按固定顺序应用引擎级注入：Metadata → Debug → Logger → Metrics（Go 无 `ResourceAware` 接口，资源在 execute 时经 ctx 注入；Java/C++ 的 Resource 注入排在 Metrics 之后）。
   - `MetricsAware` 注入使用 `NewEngine` option 中提供的 `metrics.Provider`；默认是 no-op provider。

4. **构建 DAG**（`pine-go/internal/dag.Build`）
   - 推导数据冒险边（含行集标记的 `_row_set_` 哨兵处理）。
   - 添加显式 `sources` 边。
   - 拓扑排序校验（环检测）：必须在归约之前运行，因为传递性归约在有环图上可能移除"冗余"边从而掩盖环。
...
在 item 字段遍历中，三个行集标记（marker interface）和一个自动注入机制通过 `_row_set_` 哨兵字段参与统一的冒险追踪，无需单独的屏障阶段：

- `AdditiveWritesRowSet`（Recall 算子）：将 `_row_set_` 追加到 item 写字段列表，作为追加型写者参与冒险追踪
- `ConsumesRowSet`（隐式结构性行集依赖的算子）：将 `_row_set_` 追加到 item 读字段列表，作为读者参与冒险追踪。仅在算子的 metadata 中 item 字段为空但仍然结构性地访问行集时才需要（如 `transform_size` 调用 `ItemCount()`、`transform_remote_pineapple` 无条件读取并转发 items）
- `MutatesRowSet`（Filter/Merge/Reorder 算子）：在所有字段冒险处理完成后，对 `_row_set_` 执行覆写型写入，产生完整的 WAW/WAR 边并重置追踪器
- **Auto-inject**（item 字段自动注入）：对于非 `AdditiveWritesRowSet` 且非 `ConsumesRowSet` 的算子，若其 metadata 声明了非空的 `item_input` 或 `item_output`，构建器自动将 `_row_set_` 注入为读字段。这确保有 item 字段的算子无需显式标记即可等待行集稳定，是行集依赖的主要安全网

这四个机制的组合效果替代了旧版的独立"屏障阶段"。Filter/Merge/Reorder 同时实现 `ConsumesRowSet` + `MutatesRowSet`，因此它们既读取 `_row_set_`（等待行集稳定）又对其做覆写型写入（序列化后续对行集的访问）。约束：`AdditiveWritesRowSet` 与 `MutatesRowSet` 互斥，Recall 类型必须实现 `AdditiveWritesRowSet`。
...
环检测在传递性归约之前运行。`TopologicalSort` 在完整边集上校验图是否无环——这一点至关重要，因为传递性归约在有环图上可能移除被更长路径"隐含"的边，从而掩盖本应被拒绝的循环依赖。校验通过后，再执行传递性归约移除冗余边。

环表示由行集标记、冒险或显式 source 边暗示的不可能排序。
...
`pine-go/internal/dag/dag_test.go` 现在不仅覆盖单一 hazard 或单一行集标记语义，还专门覆盖多特性交互与诊断质量。
...
- `TestNestedSubFlowWithBarrierAndSources` — 覆盖 recall + merge + filter + SubFlow 的组合，确认嵌套 SubFlow 场景下 MutatesRowSet 与显式 `sources` 边仍能共同给出正确依赖骨架。
- `TestControlFlowWithBarrierInteraction` — 覆盖控制字段依赖与 MutatesRowSet 语义交互，确认控制流守卫不会破坏行集变异建立的序列化约束。
...
跨引擎 fixture 验证 auto-inject 机制在多种 item 字段组合下的正确性，确保有 item 字段的算子即使不带显式 ConsumesRowSet 也能获得正确的 `_row_set_` 依赖边。各运行时的 fuzz 测试（如 Go `FuzzBuild`、Java）均添加了行集安全不变量断言：每个拥有 item 字段的算子在构建后的图中必须有 `_row_set_` 依赖边。

`scripts/dag-differential-fuzz.py` 提供 DAG 级别的多引擎差异模糊测试，生成随机管道配置，在各运行时中构建 DAG 后比对边集（而非执行输出），检测 DAG 构建层面的跨引擎不一致。

## 行集依赖模型（Row Set Markers）

某些算子依赖 item 集合整体而非特定 item 字段。Pineapple 通过三个标记接口和 `_row_set_` 哨兵字段建模这种依赖，无需单独的图机制。
...
- `AdditiveWritesRowSet` 算子（Recall）作为 `_row_set_` 的追加型写者。
- `MutatesRowSet` 算子（Filter/Merge/Reorder）对 `_row_set_` 执行覆写型写入，重置追踪器。
- `ConsumesRowSet` 算子作为 `_row_set_` 的读者。仅在 metadata 中 item 字段为空但仍结构性访问行集时才需要显式标记（如 `transform_size` 调用 `ItemCount()`）。
- **Auto-inject**：对非 `AdditiveWritesRowSet` 且非 `ConsumesRowSet` 的算子，若 metadata 声明了非空 `item_input` 或 `item_output`，自动注入 `_row_set_` 读。这是行集依赖的主要安全网，大多数有 item 字段的算子无需手动标记。

这捕获了集合级因果关系，例如：

- `transform_size` 等算子需要在完整 recall 行集就绪后才能计算 item 数量
- 等待产生行的 recall，而无需发明假的业务字段名
- Filter/Merge/Reorder 的行集变异必须序列化，后续读者必须看到稳定的行集

该哨兵仅限内部使用。用户流水线不应将其视为真实字段。

Go 侧在 `pine.NewEngine()` 编译算子时，通过 interface 类型断言（`types.ConsumesRowSet`、`types.MutatesRowSet`、`types.AdditiveWritesRowSet`）检测标记，并写入 `OperatorConfig` 的对应布尔字段。Java 侧使用同名 interface。
...
这把 `data_parallel` 的实例能力判定收敛到真实持有算子实例的 Go 层，替代旧的跨 Apple/Go 名称 blocklist 模式，避免双端事实源漂移。
...
DAG 基于字段级数据冒险（RAW/WAW/WAR）和行集标记建边，保证并发执行的算子访问不同字段。MutatesRowSet 算子（Filter/Merge/Reorder）通过 `_row_set_` 覆写型写入建立序列化边，不会与其他行集访问者并发。`done[pred]` channel 的 close 在 Go 内存模型下提供 happens-before 保证，前驱写入对后继可见。
...
- 三运行时的 ExecutionError/PanicError 均**保留原始异常因果链**：Go 通过 `Unwrap()` 接口，Java 通过 `Throwable.getCause()`，C++ 通过 `std::nested_exception`（多继承）+ `pine::error_as<T>()` helper。Section 15（`scripts/cross-validate/15-error-cause-chain.sh`）通过三方 probe binary 字节级验证三方 cause chain 输出 `"PASS:key=user:42 not found"` 一致
...
- 算子自身配置携带的逐算子 `debug`（nullable 三态：未设置时继承全局，显式设置时覆盖）
- `NewEngine` 根据根级 `debug` 或 `pine.WithDebug(...)` 确定的 flow-level 全局 debug 默认值

因此全局 debug 不引入新的 trace 结构；它作为未显式设置 debug 的算子的默认值，而显式设置了 `debug=true` 或 `debug=false` 的算子不受全局值影响。
...
（用例、`memory/doc-gaps.md`、`guides/investigation-to-fix-testing.md`、这里），而只有用例会在写错时
变红。**判据：一条机制描述被审计连续改错两次以上时，删掉散文副本、只留能执行的那一份加指针。**
...
`BuildInput` 将 frame 投影到算子声明的元数据契约。Go/Java/C++ 三运行时均采用 **lazy proxy** 模式（而非 eager reify）：构造时仅做 strict 字段批量校验，实际字段值在算子调用 `Common(field)` / `Item(i, field)` 时按需从 frame 读取，避免 O(N×M) 预复制开销。语义不变：
...
- 字段访问策略遵循 InputFieldSpec 三态模型：
  - **Nullable**（默认）：字段缺失 → error；值为 nil → 透传 nil
  - **Strict**（通过 `strict_common` / `strict_item` opt-in）：字段缺失或值为 nil → error
...
- `required field "X" is nil in common`
- `required field "X" is missing in common`
- `required field "X" is nil on item[i]`
- `required field "X" is missing on item[i]`

经 `ExecutionError` 包装后最终输出形如 `pine: execution error in operator "op": required field "X" is missing in common`，由 `fixtures/errors/runtime_build_input_missing_field.json` 通过 `wrapping_exact` 锁定三运行时（go/java/cpp）的字节级一致。
...
| Reorder | 改变行顺序 | 仅 `SetItemOrder` | ConsumesRowSet + MutatesRowSet |
| Observe | 只读副作用 | 无 | 有 item 字段时自动注入 `_row_set_` 读；仅当 metadata 中 item 字段为空但仍结构性访问行集时需显式 ConsumesRowSet |
...
- `debug` — 逐算子 trace 捕获开关（nullable 三态：未设置时继承全局，显式设置时覆盖）
- `row_dependency` — 已废弃，v0.7 起由 `consumes_row_set` / `mutates_row_set` / `additive_writes_row_set` 三个布尔字段替代
- `consumes_row_set` — 启用 `_row_set_` 读取（ConsumesRowSet 标记）
- `mutates_row_set` — 对 `_row_set_` 做覆写型写入（MutatesRowSet 标记）
- `additive_writes_row_set` — 对 `_row_set_` 做追加型写入（AdditiveWritesRowSet 标记）
- `common_defaults` / `item_defaults` — 快照构建时的输入默认值
- `strict_common` / `strict_item` — 字段列表，列在其中的字段在 BuildInput 时走 Strict 模式（缺失或 nil 均报错），未列出的字段走 Nullable 默认模式
...
- `pine-go/pkg/resource/` 管理命名资源，支持后台刷新和原子读取。资源分两类：**数据型资源**通过 `snapshot()` 整体导出活值并注入请求上下文（如 `recall_resource` / `transform_resource_lookup` 消费的查找表）；**句柄型资源**通过 `borrow(name)` 借出长生命周期连接句柄（如 `redis_connection`，由 `transform_redis_get` / `transform_redis_set` 按 `resource_name` 借用），不走整体快照，`interval: -1` 表示永不刷新。
...
2. **MutatesRowSet 算子序列化行集变异。** Filter/Merge/Reorder 通过 `MutatesRowSet` 标记对 `_row_set_` 做覆写型写入，保证行集变异不与其他行集访问并发。不要随意弱化它们；许多执行顺序保证依赖于此。
...
11. **注入顺序固定。** 对同时实现多个可选接口的算子，跨运行时不变量是：Metadata/Debug 信息先于一切 provider 注入到位（Go/Java 走 `MetadataAware`/`DebugAware` 接口，C++ 经 `init(op_cfg)` 直接携带），随后按 `LoggerAware` → `MetricsAware` → `ResourceAware` 注入。各运行时的完整路径：Go `Metadata → Debug → Logger → Metrics`（无 `ResourceAware` 接口，资源在 execute 时经 ctx 注入）；Java `Metadata → Debug → Logger → Metrics → Resource`；C++ `init(含 metadata/debug) → Logger → Metrics → Resource`。依赖 operator name 做 metric label 的实现可依赖 Debug 信息先于 Metrics 就位。
12. **全局 debug 通过 nullable 三态继承生效。** 根级 `debug` 与 `pine.WithDebug(...)` 确定 flow-level 全局 debug 默认值；逐算子 `debug` 为 nullable 三态（Go `*bool`、Java `Boolean`、C++ `std::optional<bool>`），未设置时继承全局值，显式设置时覆盖全局值。Go option 还必须保留”未设置”与”显式 false”两种状态区分。
13. **SubFlow 只影响可视化分组，不影响执行语义。** `Node.SubFlow` 与 collapsed render 不能改变 DAG 的边、调度顺序或 hazard 推导；它们只是对既有执行图的聚合视图。
14. **Item 字段算子自动获得 `_row_set_` 读依赖。** 非 AdditiveWritesRowSet 且非 ConsumesRowSet 的算子，若 metadata 声明了非空 item_input 或 item_output，构建器自动注入 `_row_set_` 读。这确保 item 字段算子无需手动标记即可等待行集稳定。显式 ConsumesRowSet 仅在 item 字段为空但仍结构性访问行集时需要。
...
- **注入顺序**：引擎编译算子时按 MetadataAware → DebugAware → LoggerAware → MetricsAware → ResourceAware 固定顺序注入，前四段与 Go 侧一致、末段 Resource 为 Java/C++ 特有（Go 的资源在 execute 时经 ctx 注入）
- **结构化错误**：6 种错误类型 — `ConfigError`、`RegistryError`、`ValidationError`、`OperatorException`（checked）、`ExecutionError`、`PanicError`。`Operator.execute()` 声明 `throws OperatorException`；引擎据此区分预期算子错误（→ ExecutionError）与意外运行时异常（→ PanicError），对应 Go 的 error/panic 语义。所有错误类型的 `getMessage()` 统一 `pine:` 前缀。`ExecutionError`/`PanicError` 保留 cause chain（`Throwable.getCause()`，构造时通过 `super(msg, cause)` 传入）
...
- 全部内置算子的 `execute()` 方法均接受 `CancellationToken token` 作为第一参数
...
Pine-Java 注册全部内置算子（`AllOperators.java`），与 Pine-Go `pine-go/operators/all.go` 完全对齐：

- Filter: `filter_condition`、`filter_truncate`、`filter_paginate`
- Transform: `transform_copy`、`transform_dispatch`、`transform_normalize`、`transform_size`、`transform_by_lua`、`transform_resource_lookup`、`transform_redis_get`、`transform_redis_set`、`transform_by_remote_pineapple`、`transform_bench_cpu`、`transform_bench_sleep`（后两者仅供 benchmark 使用）
...
- `--schema-from-registry` — 从内部 Registry 直接生成 Apple DSL 产物（`apple_generated/`）
