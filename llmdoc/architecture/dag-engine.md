...
在 item 字段遍历中，三个行集标记（marker interface）通过 `_row_set_` 哨兵字段参与统一的冒险追踪，无需单独的屏障阶段：

- `AdditiveWritesRowSet`（Recall 算子）：将 `_row_set_` 追加到 item 写字段列表，作为追加型写者参与冒险追踪
- `ConsumesRowSet`（依赖行集整体的算子）：将 `_row_set_` 追加到 item 读字段列表，作为读者参与冒险追踪
- `MutatesRowSet`（Filter/Merge/Reorder 算子）：在所有字段冒险处理完成后，对 `_row_set_` 执行覆写型写入，产生完整的 WAW/WAR 边并重置追踪器

这三个标记的组合效果替代了旧版的独立"屏障阶段"。Filter/Merge/Reorder 同时实现 `ConsumesRowSet` + `MutatesRowSet`，因此它们既读取 `_row_set_`（等待行集稳定）又对其做覆写型写入（序列化后续对行集的访问）。约束：`AdditiveWritesRowSet` 与 `MutatesRowSet` 互斥，Recall 类型必须实现 `AdditiveWritesRowSet`。
...
- `ConsumesRowSet` 算子作为 `_row_set_` 的读者。
...
| Transform | 变异字段值 | `SetCommon`、`SetItem` | 可选 ConsumesRowSet |
...
| Observe | 只读副作用 | 无 | 可选 ConsumesRowSet |
...
- ConsumesRowSet 的 transform 成为 `_row_set_` 读者
...
- **引擎编译流水线**：JSON 解析 → 算子序列展开（含 SubFlow）→ 标记接口检测（`ConsumesRowSet`/`MutatesRowSet`/`AdditiveWritesRowSet`）→ DAG 推导
- **DAG 推导**：与 Go/Java 相同的三阶段算法（数据冒险 + 显式 sources + 传递性归约 + 拓扑排序），通过 `_row_set_` 哨兵和三标记实现行集依赖
