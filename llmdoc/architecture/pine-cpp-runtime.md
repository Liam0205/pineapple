...
- **数据表示**
  ...
  - **`OperatorOutput` 缓冲区复用**（issue #122）：`node_body`（`src/runtime/engine.cpp`）不再每个节点新建 `OperatorOutput`，改为 `thread_local OperatorOutput` + 进入时 `reset()`。选 `thread_local` 而非对象池的理由：pine-go 用 `sync.Pool` 是因为它的调度器可能把节点交给任意 goroutine，而 pine-cpp 的 ready-queue **不会迁移半完成的节点**，`thread_local` 已足够，省掉 Get/Put 记账
    - **reset 必须在 acquire 侧而非 release 侧**——这是契约而非实现细节。`apply_output` 会 move-extract `added_items_` / `column_writes_`，留下同尺寸的 moved-from 空壳 vector；若改成在 release 侧归还前 reset，下一个节点会把这些空壳当**幽灵 item 重放**（实测删掉 acquire 侧 `reset()` 后，recall 的空壳泄进下游 Transform，触发 `Transform must not call [AddItem]` 类型违规——正是该机制的证据）。acquire 侧 reset 还顺带覆盖"抛异常的节点留下垃圾"，因此不需要写 unwind 处理
