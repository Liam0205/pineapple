...
- **数据表示**
  ...
  - **`Frame` 抽象基类**（`include/pine/frame.hpp`，R3-L3）：`ColumnFrame` 和 `RowFrame` 两个物理实现。`Operator::execute` 签名为 `const Frame&`（不绑定具体实现）。`pine::make_frame(storage_mode, common, items)` 工厂按 `Config.storage_mode` 路由（**只有精确 `"column"` → ColumnFrame，其余一切（大小写不同、空）→ RowFrame**，与 pine-go `NewFrame` 的 `default` 分支一致；issue #187 起非法值在配置加载期就被 `validate_storage_mode` 拒绝、到不了这里，但分派规则本身没变。三层解释点详见 `architecture/dag-engine.md` 的 `storage_mode` 节。issue #179 前这里写的是「其他 → ColumnFrame」，方向与现在相反）
  ...
  - **`OperatorOutput` 缓冲区复用**（issue #122）：`node_body`（`src/runtime/engine.cpp`）不再每个节点新建 `OperatorOutput`，改为 `thread_local OperatorOutput` + 进入时 `reset()`。选 `thread_local` 而非对象池的理由：pine-go 用 `sync.Pool` 是因为它的调度器可能把节点交给任意 goroutine，而 pine-cpp 的 ready-queue **不会迁移半完成的节点**，`thread_local` 已足够，省掉 Get/Put 记账
    - **两侧都 reset，承重的是尾部那次**（经过多轮审查才定型，早期版本只有 acquire 侧）：尾部 `out.reset()` 位于 `try/catch` **之外**，成功与抛异常两条路径都覆盖。抛异常的节点持有的 payload 最大（`apply_output` 根本没执行、什么都没被消费掉），早期把 reset 放在 `try` 内部时，恰恰是最坏的这条路径既不释放 payload、也逃过 `kRetainLimit`（上限是靠 reset 施加的）——实测 pool=24 时 N=70000 保留 352 MB、N=140000 保留 702 MB，随 N 线性增长无上界。acquire 侧那次现在**严格意义上是冗余的**：把它 mutate 掉全套测试依然全绿，因为尾部 reset 已经把 buffer 清干净了。保留它是 defence in depth——空容器上 `clear()` 成本为零，换来「`execute()` 一定看到干净 buffer」这条不变量在本地成立，而不必依赖 node body 里每条退出路径永远正确（`skip` 路径就是在尾部 reset 之前 return 的，只是它从不写 buffer 所以无害）。**为什么不能只有尾部那次**：`apply_output` 会 move-extract 部分容器，留下同尺寸的 moved-from 空壳 vector，必须在下一个算子执行前清掉，否则会被当**幽灵 item 重放**——回归门是 `test_output_pool.cpp` 的 failed-request 用例（两侧 reset 都删掉时报 8 个 item 而非 3 个，其中 5 个名字带 `leak`）
    - `OperatorOutput::reset()`（`include/pine/pine.hpp`）逐成员 `clear()` 而非重新赋值，因为要**保住 heap capacity**——这正是本次优化的意义所在。回归门：`tests/test_output_pool.cpp`（引擎级跨算子泄漏）+ `tests/test_operator_output.cpp` 的容量保持断言。改动这里前先读 `guides/ci-quality-baseline.md` 的"Mutation 验证的两步判据"（`= {}` 不是有效 mutation）
    - **容量保留的代价与 `kRetainLimit` 上限**：保留容量不是纯收益。`thread_local` 的生命周期是线程的，而 DAG pool 的 worker 与 Engine 同生共死（默认 `nproc * 4` 个），所以**无上限保留**意味着某次大请求撑起的容量会被每个碰过它的 worker 永久持有，没有回收时机。这与 pine-go **不对等**：Go 侧 buffer 放在 `sync.Pool` 里，GC 会清空它，高水位是会释放的（实测：cap 554598 的 output 归池后跑两次 `runtime.GC()`，再 `Get` 得到 cap=0）。因此 `reset()` 对容量超过 `kRetainLimit`（65536 元素）的 vector 做 swap-release 而非 `clear()`，把保留量封顶：生产 calibrated 情况下 N≈10、最大合成 fixture N=5000，都远在阈值之下，正常流量拿到全部复用收益，只有异常大请求会触发回收。**封顶量级要按实测值理解**：单 worker 六个被路由的成员同时打到阈值是 14.81 MiB（`item_writes_` 5.00 + `common_writes_` 4.50 + `column_writes_` 3.50 + `added_items_` 1.50 + `item_order_` 0.25 + `warning_` 0.06，按 `sizeof` 80/72/56/24/4/1 算），默认 `nproc * 4` = 96 worker 下约 1422 MiB。注意**这只是骨架预算、不含元素载荷**，真实占用在 item 带 payload 时远高于此，不可拿来做容量规划——这是"每个 worker 都恰好服务过一个 65536 量级请求"的合成最坏情况（再大就被释放了），但改阈值时应当以这个数为准。回归门：`test_operator_output.cpp` 用 SUBCASE 分别驱动 `set_item` / `add_item` / `set_item_column_double` / `set_item_order` 到超限，四个容器的释放分支**各自独立**都有断言防住（实测逐个 mutate 都变红）；保留方向由 ≤阈值的容量断言防住。**这条经验值得记住**：最初只驱动 `set_item`，另外几个分支删掉后测试仍全绿，而 recall 型负载走的正是 `added_items_`，实测能完整复现 478 MB 永不回落——单一容器的覆盖不等于该机制的覆盖。**同类漏项出现了四次**：第四个是 `item_order_`——它同样是 capacity-bearing `vector<int>`（`reorder_sort` / `reorder_shuffle_by_salt` 都按 item 数填满它），起初走裸 `clear()` 不受上限约束，实测 200000 元素（781 KiB）在 `reset()` 后原样留存。特别的是**保留它从来就不是收益**：`set_item_order` 是 move-assign，下次调用直接丢弃 `reset()` 留下的那块内存，所以它纯是成本。现在四个 vector 统一走 `clear_or_release` helper；`std::set removed_items_` 刻意不走，因为清空它是把节点还给 allocator free list，没有单一 capacity 可封顶。**第五次是 `warning_`（`std::string`）**：实测 4 MiB 在 `reset()` 后原样留存，而 warning 文案会嵌入请求数据、随请求增长。它能活过三轮审查的机制原因值得记住——`clear_or_release` 当时只接受 `std::vector<T>&`，**从类型上就把 `std::string` 排除在讨论之外**。根因是判据写错了方向：应当问「哪些成员持有随请求增长的资源」（对着成员列表逐项回答），而不是「哪些容器需要加分支」（一份要人工维护的清单，每加一个成员就多一次遗漏机会）。现在 helper 按容器自身的 `capacity()`/`clear()` 重载，`reset()` 的注释直接对 9 个数据成员逐项交代归属；`common_writes_`（FlatMap）也路由进去但刻意无测试——它没有 `capacity()` 访问器，释放不可观测，且其规模由 pipeline 配置而非请求 item 数决定。
    - **spine 与 payload 是两个维度，`kRetainLimit` 只管前者**（第四轮审查的 blocking 发现）：`capacity()` 数的是容器骨架的槽位数，而 `ItemWrite` 自己持有 `std::string field` + `Variant value`、`added_items_` 每个元素是独立堆块、`DoubleColumnWrite` 持有 `vector<double>`——这些 payload **一个字节都不在 `capacity()` 里**，因此也完全不受上限约束。叠加 reset-on-acquire（payload 活到该 worker *下一个*节点才释放），空闲 worker 会一直攥着上一次请求的整份 payload。实测 N=2000（远低于 65536 阈值、上限分支根本不触发）、每 item 4 KiB blob、400 请求后：不加 release 侧 reset 是 6.32 GB，加了是 5.36 GB，差约 960 MB。**这个 -15% 的口径会严重低估修复价值，引用时务必带上口径**：它是未 `malloc_trim` 的绝对 RSS，分母里含 frame / result 等与本修复无关的占用。换成分配器存活字节口径、只看 buffer 留存量，同一形式下是 806 MB → 34 MB（约 23x）；抛异常路径是 205.5 MB → 1.2 MB（约 170x）。**RSS 不是衡量这个修复的合适工具**——两个变体的 RSS 有时相差不到 0.1%，因为分配器把释放的块缓存住了。**即普通流量就能达到上限注释里号称是「合成最坏情况」的量级。** 修法是在 node body 尾部（`try/catch` 之外）再 `out.reset()` 一次，覆盖成功与抛异常两条路径。另外注意**哪些容器会残留取决于存储模式**，不要假定某个容器一定已被移空：`RowFrame::apply_output` 对 `added_items_` 是 move（`row_frame.cpp:269` 用 `auto&`），`ColumnFrame` 则是逐字段值拷贝（`column_frame.cpp:418` 用 `const auto&`），`column_writes_` 方向相反。实测 N=2000、每 item 4 KiB，`apply_output` 之后 `added_items_`：row 模式 0 KiB、column 模式 8003 KiB。而新增的 `transform_heavy_1000` fixture 正是钉死 column 的，所以拷贝那一侧不是假设。
    - **这条修复刻意没有单测**：保留窗口在「某节点 `apply_output` 完成」到「下一节点 acquire reset」之间，而引擎内所有可观测点都在 acquire reset 之后——算子探测 buffer 永远读到 0，无论该行是否存在（实测：写了一个这样的测试，对着删掉该行的 mutant 依然全绿，故删除，不留假覆盖）。要验证只能靠**分配器存活字节探针**——RSS 不够用，两个变体的 RSS 相差不到 0.1%（分配器把释放的块缓存起来了），这一点本身就是踩过的坑。教训：**「测不到」和「没问题」是两件事**，把不可观测性写进注释比留一个恒绿的断言诚实
