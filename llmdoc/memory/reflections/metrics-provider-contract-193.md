...
实现上有一处刻意的取舍值得记：比对的是「某运行时声明了哪一批桶数组」这个集合，而不是把每个数组映射
回它的 metric 名。三种语言的声明写法差异足以让名字关联变脆，而集合比对同样能抓到漂移、且不会静默
错配；只在两侧各有对方没有的数组时才报错，避免把「某运行时不埋这个 histogram」误判成分歧。
