...
不能接受——Redis 路径影响的是**数据正确性**（跨运行时读不到键、遗留无主键、同一个键下读到不同成员值），
不该作为 accepted regression 封存。它同时给了正确方向：**在比较点处理装箱对等，而不是改共享格式化器**。

它是对的，而且我错得比「选错了一边」更基础：**我把「共享格式化器的输出」当成了唯一可调的旋钮**，
...
- `GoFormat.sprint` **恢复**整数装箱分支 —— Go 的 `%v` 对原生 `int` 任意量级原样打印，
  ...
  三个消费者都需要这个语义。
- `FilterCondition` **自己把比较两侧归一到 double** —— Go 那边两侧都经 `encoding/json` 成了 float64，
  所以不对称本来就属于比较点。

结果四个消费者全部与 Go 一致，不再有被接受的回归。

**教训（比本次的数字问题更通用）：声称「两个约束无法同时满足」之前，先确认两个约束真的作用在同一个
可调点上。** 我连续七轮在描述这个「取舍」的作用域、归属、消费者清单，越写越精确——而它整个前提是假的。
把力气花在精确描述一个错误结论上，比结论本身更贵。
