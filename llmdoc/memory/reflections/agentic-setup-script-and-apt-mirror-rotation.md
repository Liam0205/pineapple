...
- **1 项是最锋利的**：各阶段预算之和 990s > hook 默认上限 780s，真撞上限时整个进程组被杀，**点名清单那段根本不会执行**——而 guide 里把点名清单称作"agent 唯一能知道缺哪项能力的信号"。也就是说在最需要点名的路径上，这个信号恰好缺席。修法是脚本自己维护总 deadline（读 `SETUP_HOOK_TIMEOUT`、留 45s 余量），预算不足时裁剪或跳过后续阶段，保证汇总在所有路径上都被执行到。
