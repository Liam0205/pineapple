...
原有 8 个 byte_exact fixture 里**增量部分全部**是「出事才补」的（其余四个来自通道创建提交 `b3be2503`，只覆盖最基本形状——这条在 `guides/ci-quality-baseline.md` 已按此更正）（#180 补数字格式、#183 补三个转义）。也就是
...
现在有**九个**解释点（三处分派 + 三处解析 + 三处值白名单——每层三方各一处，与 `architecture/dag-engine.md` 的三层表同口径），且白名单一侧的常量还是重复的。
...
2. **「某一侧缺一整块」这种分歧形式对代码对比阅读不可见。** 缺失在阅读中是负空间；只有穷举
   维度的矩阵能把它变成一个可见的空格子。
3. **fixture 集合按事故增长时，覆盖面永远滞后于声明。** 「字节级对等」是全局声明，而 8 个存量
   fixture 里 `01`/`02`/`04`/`05` 是通道创建时的最小集、`06`-`09` 是事故驱动补的，两者合起来仍远
   小于那句声明，差距就是未被覆盖的形状空间——`10_` 立刻证明了它非空。
...
- `architecture/dag-engine.md:477-490` 那张「六个解释点」表**现在过时了**：它描述的是三方解析层
  互不一致的基线状态，而本次已经统一，且解释点变成九个（多了三方各一处值白名单）。这条必须由 recorder 更新。
...
- **进 `guides/ci-quality-baseline.md`**（与既有的「校验通道能钉住的属性」节并列）：
  - 「**fixture 按响应形状枚举，不按事故补**」。理由用 #188 的实测：8 个存量 fixture 里一半是
    创建时的最小集、一半是事故驱动补的，按形状枚举的第一个新 fixture 立刻查出既存分歧。 配套写清 section 14 的边界——**含
...
- `make java-test`、`make cpp-test`、`go test ./...` 全过（**新增用例数不在此复述**——本行原本写的 3 / 2 在后续四个 commit 里就过期了，正是下一行那条规则说的情况）
  （**用例总数不写死**——这个数字在写下的那次提交就会过期，要数就跑命令）
- `make cross-validate` 全 PASS：section 05 error parity 含本次新增的若干 error fixture（数量以 `ls fixtures/errors/` 为准，本行原本写的 2 已过期）；
