...
### 根级配置的四处残留分歧：`debug`、重复键、键名大小写、多字段报错顺序（issue #187 审计发现）

- **现状**：#187 统一了四个根级**字符串**字段的类型处理，但审计查出四处仍不一致，都实测过：
  - **`debug`**（唯一的另一个根级标量，布尔）：pine-go 拒绝错误类型；**pine-java 的 `asBoolean()` 强转**——`1` 与 `"true"` 会真的把 debug 打开，
  `"yes"` / `[1]` 强转成 false；pine-cpp 的 `is_bool()` 守卫静默忽略、保持关闭。**三方三种行为**（实测）。修前那个样子原样保留。
  - **重复键**：同一键出现两次时 pine-go 与 pine-java 取后者、pine-cpp 的 `FlatMap` 取前者。
    `{"log_prefix":"ok","log_prefix":123}` 被前两方拒绝、被 pine-cpp 接受。
- **重复键那条是 #187 新引入的可见差异**：校验之前不看类型，取哪个重复值结果都一样；现在校验
  依赖于哪个值胜出。实际风险低（无生成器产出重复键，`apple/compiler.py` 用 dict），但它确实是
  「同一份配置在一个运行时通过、在另一个被拒」。
  - **键名大小写**：pine-go 的 `encoding/json` 精确匹配失败后会忽略大小写回退匹配 struct tag，另两方精确匹配。`{"STORAGE_MODE":"colunm"}` 被 pine-go 拒绝、被另两方接受（实测）。与重复键同理，也是本次改动新引入的可见差异。
  - **多字段报错顺序**：两个以上根级字段同时类型错时，报错点名的是哪个字段可观察。pine-java 与 pine-cpp 已对齐（同一检查顺序，由两侧单测钉住），但 **pine-go 无法用任何固定顺序匹配**——`encoding/json` 点名的是**在 JSON 文档里最先出现**的那个错误字段，答案随输入键序变化（实测：同样三个坏字段、两种键序，pine-go 分别点名 `log_prefix` 与 `storage_mode`）。这也是它不能做成 cross-validate error fixture 的原因：section 05 要求三方匹配同一子串。
- **待决策**：(a) `debug` 是否按同一规则对齐——改动小，但与 #187 一样属用户可见契约变更；
  (b) 重复键的解析语义是否统一——需要动 pine-cpp 的 `FlatMap` 插入语义或在解析层显式拒绝重复键，
  影响面比 (a) 大得多，且 JSON 规范本身对重复键未作规定。
- **不在 #187 范围内**：#187 的授权范围是「统一所有根级**字符串**字段的类型处理」，两条都超出。
