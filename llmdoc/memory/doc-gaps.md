...
### 根级配置的两处残留类型分歧：`debug` 与重复键（issue #187 审计发现）

- **现状**：#187 统一了四个根级**字符串**字段的类型处理，但审计查出三处仍不一致，都实测过：
...
- **重复键那条是 #187 新引入的可见差异**：校验之前不看类型，取哪个重复值结果都一样；现在校验
  ...
  - **键名大小写**：pine-go 的 `encoding/json` 精确匹配失败后会忽略大小写回退匹配 struct tag，
    另两方精确匹配。`{"STORAGE_MODE":"colunm"}` 被 pine-go 拒绝、被另两方接受（实测）。
    与重复键同理，也是本次改动新引入的可见差异。
