...
- `llmdoc/memory/reflections/wangshu-borrow-optimization-survey.md` — wangshu borrow/边界优化空间调查复盘（纯调查+原型量化，未改生产代码），记录 borrow 层刻意对称是设计、Arena 列轨 ABI 边界口径 -22%(N=100)~-46%(N=3000) 但端到端会稀释+落地破四引擎 parity、makeArrayTable 的 N=1000 rehash 悬崖意外发现、Boundary 微基准≠calibrated 裁判的口径纪律、非单调性是红旗须先查根因。
