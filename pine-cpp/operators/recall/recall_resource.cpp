...
public:
    void init(const OperatorConfig& cfg) override {
        ...
        if (rn_it == params.end() || !rn_it->second.is_string())
            throw ExecutionError(cfg.name, "recall_resource: missing resource_name");
        resource_name_ = rn_it->second.as_string();
    }
    void execute(const Frame& frame, OperatorOutput& out) override {
        if (!frame.resources())
            throw ExecutionError(op_name_, "recall_resource: no resource provider in context");
        auto res_it = frame.resources()->find(resource_name_);
        if (res_it == frame.resources()->end())
            throw ExecutionError(op_name_, "recall_resource: resource \"" + resource_name_ + "\" not found");
        const auto& resource = res_it->second;
        if (!resource.is_array())
            throw ExecutionError(op_name_, "recall_resource: resource \"" + resource_name_ + "\" is not an array, want []map[string]any");
        for (std::size_t i = 0; i < resource.as_array().size(); ++i) {
            const auto& elem = resource.as_array()[i];
            if (!elem.is_object())
                throw ExecutionError(op_name_, "recall_resource: items[" + std::to_string(i) + "] is not an object, want map[string]any");
