...
// unsafeNoopTestOp is a noop that does NOT implement ConcurrentSafe,
// used to test that data_parallel rejects such operators.
type unsafeNoopTestOp struct{}

func (o *unsafeNoopTestOp) Init(params map[string]any) error { return nil }
func (o *unsafeNoopTestOp) Execute(_ context.Context, _ *types.OperatorInput, _ *types.OperatorOutput) error {
	return nil
}
...
func TestDataParallelValidation(t *testing.T) {
	tests := []struct {
		...
		{"Not ConcurrentSafe dp=2", "transform_normalize", 2, nil, true},
